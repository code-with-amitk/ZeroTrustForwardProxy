...
var httpscenarios = []scenario{
	...
	},*/
	{
		...
		authHeader: "Bearer " + getJWTToken("alice"),
		expectCode: http.StatusOK,
	},
}

var dlpscenarios = []scenario{
	{
		//POST / HTTP/1.1
		//Host: example.com
		//User-Agent: ztfp-client/1.0
	...
	/*
		{
			...
			method:     http.MethodPost,
			user:       "mcp-agent",
			targetURL:  "http://mcp-provider1.com",
...
func runGroup(client *http.Client, scenarios []scenario) {
...
func main() {
	...
	// Create 2 variables
	// proxy. default value="http://127.0.0.1:8080"
	...
	// mcpJWTToken, err := jwt.GenerateJWT(GetLogger(), "mcp-agent", 1)
	// if err != nil {
	// 	logger.Error("generate MCP JWT:", err)
	// }

	jwtToken := getJWTToken("alice")
	...
	if err != nil {
		logger.Error("client init error: %v\n", err)
		return
	}

	logger.Info("Running %d HTTPS scenarios via proxy %s\n\n", len(httpscenarios), *proxyAddr)
	runGroup(client, httpscenarios)

	logger.Info("Running %d DLP scenarios via proxy %s\n\n", len(dlpscenarios), *proxyAddr)
	runGroup(client, dlpscenarios)

	logger.Info("Running %d MCP scenarios via proxy %s\n\n", len(mcpscenarios), *proxyAddr)
	runGroup(client, mcpscenarios)
}
